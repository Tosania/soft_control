...
if __name__ == "__main__":
    ...
    trainer.run(
        ...
        env_config={
            "reward_weights": {
                "dist": 3,  # 原版: 2.0 * r_dist
                "smooth": 1.0,  # 原版: r_smooth = -0.5 * norm (系数归入权重)
                "effort": 0.4,  # 原版: r_effort = -0.5 * norm (系数归入权重)
                "shape": 0.2,  # 原版: r_shape = -0.4 * norm  (系数归入权重)
                "damping": 0,  # 原版: 内部系数2.0保留，此处权重设为1.0以保持总乘积不变
            },
            "reward_scales": {
                "dist_scale": 50.0,  # 原版: np.tanh(5.0 * distance)
                "vel_penalty_scale": 0.1,  # 原版: self.vel_penalty_dist_scale = 0.1
            },
            "limits": {
